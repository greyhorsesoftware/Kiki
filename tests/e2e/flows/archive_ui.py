...
def run(ctx):
    ...
    back = wait_for(lambda: snapshot(root + "/work") if "work" in snapshot(root) else None)
    c.check("extract here restores the same tree", back == original, "differs" if back else "nothing extracted")
