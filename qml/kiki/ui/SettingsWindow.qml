...
FloatingWindow {
    ...
    Component { id: ai; Column { spacing: 12
        ...
        Text { text: status.configured ? "Jarvis is ready · " + status.provider + (status.mode === "cli" ? " · runs " + status.cli + " (its own login)" : " · API · credential from " + status.source) : "Jarvis is not configured for " + (status.provider || "…"); color: status.configured ? Kiki.Theme.green : Kiki.Theme.muted; font.family: Kiki.Theme.mono; font.pixelSize: 12 }
        Row2 { label: "Mode"; Choice { options: ["auto", "cli", "api"]; value: Kiki.Settings.jarvis.mode || "auto"; onPicked: v => Kiki.Daemon.request("AiConfigure", { mode: v }, () => { Kiki.Settings.load(); refresh(); sw.saved() }) } }
        Text { text: "cli = run the provider's own tool in print mode (claude -p, codex exec, gemini -p) with the file, using its login; api = call the provider directly with a key; auto = cli when the tool is installed" + (status.cliAvailable ? " (" + status.cli + " found)" : " (no tool found)"); color: Kiki.Theme.muted; font.family: Kiki.Theme.mono; font.pixelSize: 11; wrapMode: Text.WordWrap; width: 560 }
        Row2 { label: "Custom CLI command"; Rectangle { width: 320; height: 30; radius: 2; color: Kiki.Theme.bgDark; border.width: 1; border.color: Kiki.Theme.gutter
