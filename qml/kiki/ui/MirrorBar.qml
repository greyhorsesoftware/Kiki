...
Rectangle {
    id: bar
    property Kiki.Pane left
    property Kiki.Pane right
    property string home: ""
    property var lastMirrored: null      // ms since epoch, or null
    signal swap()
    signal mirror(bool upload)
    signal options(point pos)
    readonly property string remoteName: { const u = right && right.uri.startsWith("file://") ? (left ? left.uri : "") : (right ? right.uri : ""); const m = u.match(/^[a-z]+:\/\/([^/]+)/); return m ? m[1] : "" }
    readonly property bool localLeft: left && left.uri.startsWith("file://")
    ...
    Row {
        ...
        Text { anchors.verticalCenter: parent.verticalCenter; text: bar.left ? Kiki.Format.display(bar.left.uri, bar.home) : ""; color: Kiki.Theme.fgDim; font.family: Kiki.Theme.mono; font.pixelSize: 12; elide: Text.ElideMiddle; width: Math.min(implicitWidth, 260) }
        Rectangle {
        ...
        Text { anchors.verticalCenter: parent.verticalCenter; text: bar.right ? Kiki.Format.display(bar.right.uri, bar.home) : ""; color: Kiki.Theme.fgDim; font.family: Kiki.Theme.mono; font.pixelSize: 12; elide: Text.ElideMiddle; width: Math.min(implicitWidth, 260) }
