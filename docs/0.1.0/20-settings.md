...
|---|---|---|
| **General** | default view; sort and order; folders first; show hidden files; inspector on by default; relative dates; show hidden files (default for new panes); remember view per folder (with Forget all); list columns (Modified, Size, Kind, Accessed); heat source and the atime caveat (plan 22); confirm before remote delete; theme: follow Omarchy or pick from the eight themes; font size | `settings.toml [view]`, `[theme]`, plan 21 `ViewPrefs` messages |
