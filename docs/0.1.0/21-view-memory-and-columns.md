...
- **What is remembered**: view mode (icon, list, columns), sort role and sort order, and whether hidden files are shown (plan 23), per exact folder. Nothing is inherited by subfolders: a photo folder's icon view should not leak into its `raw` subfolder.
