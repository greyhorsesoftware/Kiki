...
- **Plugins are separate executables** of three kinds, all on the same framing: location plugins (`kiki-plugin-<scheme>`, plan 06 and 17), share plugins (`kiki-share-<id>`, plan 18), and Open in entries, which are command templates rather than processes (plan 14).
- **Helper processes** (`kiki-helper-dbus`, `kiki-helper-highlight`, `kiki-helper-ai`) live under `/usr/lib/kiki/helpers/` and speak the plugin framing; the D-Bus helper hosts FileManager1 and the portal backend with zbus, the highlight helper carries tree-sitter, the AI helper carries TLS. None of those dependencies enter kikid.
...
Tabs, bulk rename, further location plugins (S3 and other object stores, WebDAV, SMB), iPhone app document folders, bidirectional sync, following symlinks, mirroring permissions.
