...
| 18 | `18-share.md` | 4, 5, 6, 14 | Share menu with share plugins as processes; Mail, Tailscale (Taildrop) and LocalSend (Messages and AirDrop were removed before release) (OpenDrop over OWL) shipped (ships after 14, before 10) |
...
| 29 | `29-release-readiness.md` | all | What stands between the tree and 0.1.0: Mirror view renamed Side by Side with its own toolbar button and a draggable divider, cut loose from view preferences, Side by Side and mirroring driven by hand and then by tests, the location kinds a build actually ships, large transfers in three directions, drag and drop between the panes with modifiers, local/remote defaults and spring-loaded folders, the activity orb in the window's lower right corner, the pointer path, the manual checklist, open questions, housekeeping (last, before tagging) |
| 30 | `30-code-health.md` | all | What the 2026-09-19 code scan left open, in order: LocalSend verifying the peer it sends to, the daemon's three unbounded caches (mirror plans, git status, git roots), the theme's 2 s poll, and the duplication (`Label`, `Rule`, Shell's view components). |
