...
- **Messages and AirDrop are gone** (plugins, workspace members, PKGBUILD install line and optdepends, docs, design mockup). Three remained: **LocalSend, Mail, Tailscale**. Reasons in `18-share.md`.
- **LocalSend could never have sent to a real device** — no client certificate, discovery blind beside the app Omarchy ships, multicast-only. Fixed and checked against the running app; details in `18-share.md`. The send to a **phone** was never made, and on **2026-09-21 the owner removed LocalSend from 0.1.0** the same way Messages and AirDrop went. **Two share plugins ship: Mail and Tailscale**, and no phone test is owed.
- **No "Share ▸" any more.** The context menu has the ways themselves, between separators — *Send via Mail*, *Send via Tailscale ▸* — and a plugin's targets are the submenu of its own row, asked for when the row is opened (`ContextMenu`'s `load`; "Looking…" meanwhile). Choosing a plugin used to open its targets as a second menu at the top of the window. `Alt+S` still opens the same rows under the toolbar. Looked at in the running app with a real right-click; `tst_MenuLazySub.qml`.
...
- **LocalSend shipped switched off** (`off_by_default` in its `Describe`), waiting for that phone. With it gone, **nothing ships switched off** and no plugin can ask to: `off_by_default` / `defaultEnabled` had no other user and went with it (SDK, `share.rs`, `share_contract.rs`). An installed share plugin is on until the user switches it off.
