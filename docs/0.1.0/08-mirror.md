...
**Memory of the three collections** (from the plan-26 benchmark): the master map, the replica map and the plan each hold one entry per path, and the naive port carried an owned `String` per path in the map key, again inside each `Entry`, and again in each `Action`, so a 200,000-file scan held four copies of every path. The engine now allocates each relative path once as an `Arc<str>` shared by the key, the entry and the action, which cut the scan's footprint by roughly half with no change to the diff, its tests or the protocol. The next step, if very large trees become common, is a true arena: one `Vec<u8>` of paths with `u32` offsets, the maps as sorted index arrays, and the diff as a merge of two sorted arrays; that removes the per-path allocation and the `BTreeMap` node overhead as well, at the cost of rewriting `scan_side`, `diff`, `execute` and the report around indices.
