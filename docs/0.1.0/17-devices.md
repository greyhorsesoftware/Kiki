...
**Status:** not in 0.1.0 (plugins removed from the tree 2026-09-23; detection off).
...
**Where the code stands (2026-09-23).** ~~**Status**: generated.~~ The three plugins were generated — `kiki-plugin-mtp`, `kiki-plugin-ptp` and `kiki-plugin-afc`, each linking its C library through hand-written `extern "C"` blocks (no bindgen, no build script) — and **left the tree on 2026-09-23** (owner): they were not in 0.1.0, never ran against hardware, and a CI job whose only purpose was to keep unshipped code compiling was not worth its minute and four libraries. They are in git history; `406e224` is the last commit that has `plugins/kiki-plugin-{mtp,ptp,afc}`. ~~They are workspace `members` but **not `default-members`**, so a plain `cargo build` does not build them and the PKGBUILD installs none of them; one CI job still builds them with `-p` and installs their libraries, or they would rot unseen.~~ `plugin::LOCATION_KINDS` is `ftps`, `sftp`, `smb` — `plugin::ships("mtp" | "ptp" | "afc")` is false — so **detection is off**: `devices::start` does not spawn its thread when no device kind ships, `devices::openable` drops any device whose kind this build does not ship, and the sidebar draws no Devices section (tested, with an assertion that sends whoever ships a device kind back here). None of the three has ever been run against real hardware.

**Bringing one back** is: `git checkout 406e224 -- plugins/kiki-plugin-<kind>`, then its entry in the workspace's `members`, its kind in `plugin::LOCATION_KINDS`, its line in the PKGBUILD's install loop, the device libraries back in `depends` and in CI's install line — and expect the SDK to have moved under it. **First build on Omarchy**: compare the struct layouts (`LIBMTP_file_t`, `LIBMTP_devicestorage_t`, `CameraFileInfo`) and the constants (`LIBMTP_FILETYPE_UNKNOWN`, `LOCKDOWN_E_*`) against the installed headers before trusting a listing. Hotplug is a two-second sysfs rescan rather than a netlink listener (same result, no socket code); the daemon side is `kikid/src/devices.rs`.
