...
**As built (2026-09-23)**, `.github/workflows/ci.yml` has two jobs on every push: daemon, plugins **and the package** per architecture (`make lint`, `make test-rust`, the ignored protocol and contract tests, then `makepkg` into the same cached `target/`, the package and its `.sha256` kept as artifacts); and QML under Qt with no compositor. **The e2e flows under `cage` against the *installed* package** moved to `release.yml` the same day (owner), which is run by hand: it takes a CI run's packages, and its job "Install the package and drive it" runs them before the publish job tags and releases — so the drive gates every release, nothing is built twice, what ships is what CI tested, and no push releases anything. It was the slowest job by a margin and the one most likely to go red for reasons that were not the code; the three that stay are fast, cached and deterministic. There is no visual, layout or performance job; the benchmark job that existed until 2026-09-23 compared a GitHub runner against the owner's machine and could only fail (plan 26).

- `cargo test` on every push.
- `qmltestrunner` on every push.
- ~~e2e, visual, layout and performance in one job under `cage`~~ ~~e2e in one job under `cage`, on every PR, about ten minutes.~~ e2e under `cage` on every release and by hand (`release.yml` with publish off), not on every push.
