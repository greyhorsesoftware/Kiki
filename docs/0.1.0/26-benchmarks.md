...
- **How to run it so the numbers mean something (2026-09-23).** The baseline machine is a hybrid CPU (four P-core threads, eight E-core), and an unpinned run lands its single-threaded phases on either at random: `index_query_us` read 398 µs pinned and anywhere from 450 to 1160 µs not, and a whole-suite compare flagged a dozen lines that were nothing but scheduling. So: `taskset -c 0-3 kikid bench run …`, and compare against a **fresh run of the baseline commit on the same day** (`git worktree add /tmp/base 6a6fc71 && cargo build --release -p kikid` there), not only against the checked-in JSON — the machine is not the same machine it was the day the JSON was written. `strace` and `perf` (`sudo pacman -S perf strace`; build with `CARGO_PROFILE_RELEASE_STRIP=false CARGO_PROFILE_RELEASE_DEBUG=1` to see symbols) are what found both of the real ones below.
- **First run, 2026-09-23**, HEAD against 6a6fc71 both pinned: two real regressions, both fixed. **`flat200k.enrich_ms` 140 → 240 ms**: `enrich_all` had started queueing rows in pool (readdir) order instead of view order — a correctness fix, so that a filtered listing enriches every row and a `Sort` waiting on it completes — and stat'ing a 200,000-entry btrfs folder in readdir order is 40 per cent slower than in name order. Found by bisecting (the first commit after the baseline, `8532d33`). Now the view first, in its order, then what the filter hides. **`index_query_us` 315 → 590 µs**: `query` read every entry's name inside the scan even for a prefix query, which only reads the key — two more arrays of cache misses per name over a memory-bound loop. Now 398 µs; the rest of the gap is ranking every match instead of the first 40,000 (the fix that motivated the change), and within plan 12's 1 ms. Also seen and left: `photos.window_meta_ms` 1.6 → 4 ms (18 ms in a full run) is the thumbnail cache lookup per visible row, a stat each, since thumbnailing went to its own process — one-time, first paint of a picture folder, not worth a cache; `photos.thumbs_ms` 84 → 0.6 ms is the same change paying for itself.
