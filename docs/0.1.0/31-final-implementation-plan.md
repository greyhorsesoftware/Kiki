...
**Still to do in this phase**, as below: extract-undo and the archive rules; the traversal test; cancel-ends-as-done for trash and delete; share's temp directory; search's `capped` and `Unsupported`; git (columns, the `.git` watch, the two dead settings, inspector detail); launch and packaging; plan 29 F; the views; project mode; `views.toml`; the two events and the `Hello` check.
