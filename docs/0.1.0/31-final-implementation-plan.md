...
### Phase 4 — large transfers (3½–4 days) — **in progress**
...
**Still to do in this phase**: the remote half of "extend `remote_transfers`" below — remote `delete`, `rename` and `mkdir` as jobs, collisions (replace, keep both) against a server, mtimes compared. **For phase 8, by hand:** one real FTPS server that is not vsftpd (ProFTPD or FileZilla Server) — the session-reuse fix is proven against vsftpd only. **For the release notes:** a name that is not valid UTF-8 cannot be transferred; a name with a line break cannot be stored over FTP. The harness now kills any server a flow did not live to stop (`KIKI_E2E_PIDS`): five `sshd`s had been left behind by runs that were killed mid-flow.
