...
**Still to do in this phase**, as below: the views (gallery's `Del`, count, type-ahead and the folder with no pictures; a columns drag carrying the selection); project mode's four defects; the `views.toml` clean-up; emitting `FavoritesChanged` and `Gone`, and the `Hello` version check.
