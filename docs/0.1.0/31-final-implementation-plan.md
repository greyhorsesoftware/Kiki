...
- **A refused target is only what the code can know.** Plan 29 wants the "no" cursor for "a read-only location", but no location carries such a flag and nothing asks a plugin whether it may write. Refusal during a drag therefore means what `dropAction` already computes — a folder into itself, an item into the folder it is already in — plus remote-onto-trash. A read-only location waits for the flag, post-0.1.0.

**To build, in this order.** The first two come first for a reason: one makes everything after it testable the day it is written, the other is the only thing in this phase that can lose files.

1. **IPC `shell drop <uris-json> <dest> <modifiers>`**, calling `Pane.dropInto` with a drop-shaped object — the pattern `sideBySide drag` already uses (`Shell.qml:834`) — and **`tests/e2e/flows/drag_between_panes.py`** on it: every local / SFTP / FTPS pair and each modifier, asserted on disk, under `cage`.
2. **Trash refuses remote files.** `Sidebar.qml:127` submits `trash` for any URL it is handed; a remote has no trash, so dropping a server's files on it today is a silent **delete**. It asks first, in danger colours.
3. **Gallery as a drag source.** It has the `DropArea` and no `Drag`: what `ListRow.qml:96-104` does — `Drag.Automatic`, mime from `pane.dragMime`, threshold 8.
4. **Columns drags the selection**, through `pane.dragMime` like the other two.
5. **`Alt` and a right-button drag**: drop, then a menu at the pointer — Copy here / Move here / Cancel. **Spiked in the first hour**, before anything is built on it: plan 29 assumed a right-button drag arrives through `Drag.Automatic` and nobody has yet seen one. If the compositor does not deliver it, the menu needs another trigger and this item's estimate moves.
6. **Shown while dragging**: a one-line hint by the pointer ("Copy to homelab:/srv/site") and the cursor badge, changing the moment a modifier goes down or the target changes; modifiers read continuously and again at drop, never captured at the start. A refused target shows the "no" cursor *during* the drag, not at the end of it.
7. **Focus and cancel**: the drop moves focus to the pane that received it; the press that starts a drag neither focuses the other pane nor disturbs the selection being dragged; `Esc` cancels and changes nothing.
...
**Tests**: `tst_DragDrop` / `tst_DropAction` grow the ask, refused targets, remote-to-trash, modifiers read at drop time, focus to the receiver, and the gallery and columns sources carrying a selection. `drag_between_panes.py` as above. What stays manual is the physical press–move–release, and it goes on the phase 10 checklist.
