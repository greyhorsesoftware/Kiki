...
**Still to do in this phase**, as below: launch and packaging; plan 29 F (binding loop, Theme polling, the portal warning, the video player, `jobs::wait`); the nvim socket; the views (gallery, columns drag); project mode; `views.toml`; the two events and the `Hello` check.
