...
### Seen while running the app (owner, 2026-09-20) — to be looked into before phase 5 is closed

- **Gallery: thumbnails turn into generic file icons.** A picture that had its thumbnail in the filmstrip (or on the stage, for a remote one) goes back to the plain file artwork. Not yet investigated — find out *when* first (after a delete? a rename? a file arriving in the folder? a git command in a terminal? simply scrolling the strip away and back?). Two places to start, both from reading and neither confirmed:
  - `listing/scan.rs` `rescan()` **clears every thumbnail the listing holds** (`inner.thumb.clear()`, and `thumb_queued` reset) whenever the folder is re-enumerated — which the watcher does on a burst of changes, and which a job that touches the folder causes. The thumbnail files are still in the cache directory, but a row gets its `thumb` back only when a stat job runs for it *while it is inside a live window*; a row already enriched is not queued again, so its thumbnail may never be asked for a second time. If that is it, every view loses thumbnails after a rescan and the gallery is merely where it shows most.
  - The filmstrip's fallback changed on 2026-09-19 from the stroke glyph (`UI.Icon`) to the artwork (`UI.KindIcon`), so a row *without* a thumb now looks like "a generic file" where it used to look like a small outline — the change may have made an older fault visible rather than made a new one. `GalleryPane` also tells the daemon its viewport by the strip's position only (`syncStrip`), so rows outside it are never thumbnailed; check that the strip's delegates re-read `row.thumb` on `rowsUpdated`.
  - Wanted: a test that lists a folder of pictures, waits for thumbs, forces a rescan (touch a file), and asserts the thumbs are still — or again — there; then the same through the gallery's strip.
