...
**SFTP** (`russh` + `russh-sftp`): fields Name, Host, Port (22), Username, Identity file (with passphrase in the keyring), Password (alternative), Remote path, Local path. `Read` and `Write` keep 16 requests in flight (SFTP throughput is bounded by round trips, not bandwidth, with one outstanding request), and readdir attributes fill `Meta`.
...
- **Probe**, once per `Connect` and cached with the session: open an exec channel and run `command -v find stat sh && find --version 2>/dev/null | head -1`. Outcomes: `gnu` (GNU findutils present), `posix` (a `find` without `-printf` but a usable `stat -c` or `stat -f`), or `none` (exec refused, restricted shell, `ForceCommand internal-sftp`, no `find`). The probe has a 3 s timeout; any failure means `none`. The result is reported in `Capabilities` as `fastScan: "gnu" | "posix" | "none"` so the UI can show it in the location's tooltip.
- **Single directory** (`Scan`): with `gnu`, run `find <path> -mindepth 1 -maxdepth 1 -printf '%y\0%Y\0%s\0%T@\0%m\0%u\0%g\0%f\0'` and parse NUL-separated records: type, link target type, size, mtime with fraction, octal mode, owner, group, name. Filenames with spaces or newlines are safe because every field is NUL-terminated. With `posix`, run `ls -1A` piped through `stat` in a batch and parse; with `none`, use SFTP `READDIR`. Paths are single-quote escaped before being placed in the command; nothing from a filename is ever interpolated unquoted.
...
- **Fallback** is per call: if the exec channel fails mid-stream (connection limit, killed process, non-zero exit), the plugin logs it, marks `fastScan` as `none` for the rest of the session and repeats that call over SFTP. Results from a partial exec stream are discarded, never merged.
